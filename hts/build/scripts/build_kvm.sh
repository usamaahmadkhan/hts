...
if [[ $string == *"Red Hat"* ]]; then
  ...
  bash /root/hts/hts/build/scripts/build_kvm_centos.sh
elif [[ $string == *"Ubuntu"* ]]; then
  log "Running build_kvm.sh for Ubuntu"
  bash /root/hts/hts/build/scripts/build_kvm_ubuntu.sh
  else
      log "Running build_kvm.sh for SUSE"
      bash /root/hts/hts/build/scripts/build_kvm_suse.sh
fi
